...
import json, copy, re, sys, io, os, wave, contextlib, math, struct, hashlib
...
# 図レイアウトリンター(figlint)の共有登録＝カード合成のラベルbboxもfiglintで検査可能にする（ロケーター等の"偽の緑"潰し・2026-08-15）
try:
    from gen_medical_schematics import _lintreg, _LINT as _MLINT
except Exception:
    _MLINT = {"on": False}
    def _lintreg(*_a, **_k): pass

def _code_sig(fn):
    """関数のバイトコード＋定数(色/座標/文字列)の指紋。描画コードや定数を変えると変わる。
    キャッシュキーに混ぜると『コードを直したのに古い図が返る』stale を自動で無効化できる（今日の誤報の根治・2026-08-15）。
    ※コメント/docstring変更でも変わりうる＝過剰無効化は"一度だけ同じ図を再生成"で無害。"""
    import hashlib as _h
    c = fn.__code__
    parts = [c.co_code]
    for k in c.co_consts:
        parts.append(getattr(k, "co_code", repr(k)))   # ネストcodeはそのバイトコード、他はrepr
    return _h.md5(repr(parts).encode("utf-8")).hexdigest()[:8]

def _card_out(prefix, fn, src, extra=""):
    """カードラッパーの出力パス＝『関数コード指紋＋src画像のmtime＋extra』でキー化。
    内側画像(src)が更新されれば必ず別名になり、ラッパーの消し忘れによるstaleを物理的に防ぐ。"""
    import hashlib as _h
    key = f"{Path(src).stem}|{_code_sig(fn)}|{_mtime_sig(src)}|{extra}"
    return FX / f"{prefix}_{_h.md5(key.encode('utf-8')).hexdigest()[:12]}.png"
...
COMPOSITIONS = {
    "ep01_nis_pump_automatic_door_3d": {
        ...
        "main_simple": r"医療\細胞_分子\NIS機構_模式図_simple.png",   # Bが納品したら本編は自動でこちら（詳細マスターは温存）
    ...
    "ep01_tpo_enzyme_glue_assembly_3d": {
        ...
        "main_simple": r"医療\細胞_分子\TPO機構_模式図_simple.png",   # Bが納品したら本編は自動でこちら（詳細マスターは温存）
    ...
    "ep01_thyroid_what_and_where_curious": {
        ...
        "left_max": [580, 520], "left_center": [380, 610],        # 原発も大きく（ユーザー要望）
        "right_max": [880, 660], "right_center": [1380, 610],     # 用量テーブルは主役＝大きく（文字可読性・#6）
        "arrow": [690, 610, 900], "arrow_w": 12, "arrow_head": [34, 20],  # 矢印も大きく・太く
        "q_size": 110, "q_pos": [740, 476],
    },
    "ep01_butterfly_thyroid_anatomy_3d": {   # id3：のどの下＋蝶の形（台詞と一致）＝本体ロケーター＋チョーク注記
        "layout": "locator",
        "title": "甲状腺は「のどの下」にある", "subtitle": "羽を広げた蝶のような形の小さな臓器",
        "body": r"医療\解剖_臓器別\上半身_首_フラット色_AI生成透過.png",
        "organ": r"医療\解剖_臓器別\甲状腺_servier模式図.png",
        "mark": [0.499, 0.824], "organ_label": "甲状腺", "locator_label": "のどの下（首の付け根）",
        "note": "羽を広げた蝶のような形",
    },
...
DIAG_PX_PANEL = (475, 108, 1420, 800)    # 左に用語解説/豆知識パネルがある行用。パネル既定縮小(PANEL_SCALE)で左端X45〜約455に細くなった分、図枠を左へ広げ拡大（ユーザー要望2026-08-14）
TITLE_PX = (50, 50)
...
def _expand_home(obj):
    # テンプレJSONは %USERPROFILE% トークンで保存（ユーザー名をハードコードしない）。
    # 読込時に実ホームへ展開＝出力.ymmpには実パスが入りYMM4が解決できる。
    # ※置換は明示トークンのみ（"100%"等の一般文字列を壊さないため os.path.expandvars は使わない）。
    if isinstance(obj, str):
        return obj.replace("%USERPROFILE%", _HOME)
    if isinstance(obj, list):
        return [_expand_home(x) for x in obj]
...
def make_framed_card(src, top_band=True, bot_band=True):
    ...
    FX.mkdir(exist_ok=True)
    out = _card_out("card", make_framed_card, src, f"{int(top_band)}{int(bot_band)}")   # コード指紋+src mtimeでstale根治
    if out.exists(): return out
...
def _compose_locator(spec):
    ...
    FX.mkdir(exist_ok=True)
    # sigに"この関数のソース"を含める＝描画コードを変えたら自動でキャッシュ無効化（消し忘れ=記憶頼みを排除。2026-08-14）
    sig = hashlib.md5((repr(sorted(spec.items())) + inspect.getsource(_compose_locator)).encode("utf-8")).hexdigest()[:10]
    out = FX / f"_locator_{sig}.png"
    if out.exists() and not _MLINT.get("on"): return out   # figlint録画中はキャッシュ短絡せず必ず描画＝ラベルbboxを記録する
    W2, H2 = 1920, 1080
    ...
    _lintreg("shape", ox0, oy0, ox0+organ.size[0], oy0+organ.size[1], "臓器図", 0)   # figlint:ラベルが臓器に被らないかの検知用
    ...
    if olabel:
        ...
        _ow=d.textlength(olabel,font=_of)
        # ラベルは臓器の絵に重ねない＝ズーム枠の"外側(上)"へ黒チップで置く（ユーザー指摘2026-08-14「文字ラベルと図形にかぶらんように」）
        _ox=fx0+2; _oy=max(204, fy0-_of.size-24)                    # 枠上端の外側。ただしタイトル下線(196)は割らないようクランプ
        d.rounded_rectangle([_ox-14,_oy-6,_ox+_ow+14,_oy+_of.size+10],radius=11,fill=(12,30,24,236),outline=CHALK+(170,),width=2)
        d.text((_ox,_oy),olabel,font=_of,fill=CHALK+(255,))
        _lintreg("label", _ox-14, _oy-6, _ox+_ow+14, _oy+_of.size+10, olabel, _of.size)
    if mx is not None:
        rad=48
        d.line([(mx+rad,my),(fx0,ocy)],fill=CHALK+(210,),width=3)
        d.ellipse([fx0-6,ocy-6,fx0+6,ocy+6],fill=CHALK+(255,))
        _MARKGRAY=(194,203,214)                                     # 位置円＝TPO図の拡大枠と同じ明るめグレーに統一（ユーザー要望2026-08-14）
        d.ellipse([mx-rad,my-rad,mx+rad,my+rad],outline=_MARKGRAY+(255,),width=6)
        d.ellipse([mx-6,my-6,mx+6,my+6],fill=_MARKGRAY+(255,))
        ...
        if llabel:
            ...
            _lintreg("label", lx0, lby, lx0+lw+lpad*2, lby+lbh, llabel, lf.size)
    ...
    if _note:
        ...
        _lintreg("label", nx, ny, nx+nw, ny+nf.size, _note, nf.size)
    ...
    if not _MLINT.get("on"):   # figlintの録画中はPNGを保存しない（mtimeを乱さない）
        base.save(out)
    return out
...
def _compose_duo(spec):
    ...
    sig = hashlib.md5((repr(sorted(spec.items())) + "|" + _srcs + "|" + _code_sig(_compose_duo)).encode("utf-8")).hexdigest()[:10]
    ...
    _lmax=spec.get("left_max",[540,470]); _rmax=spec.get("right_max",[600,500])
    _lcx,_lcy=spec.get("left_center",[430,560]); _rcx,_rcy=spec.get("right_center",[1360,560])
    lim=_fit(Image.open(lp).convert("RGBA"),_lmax[0],_lmax[1]); lcx,lcy=_lcx,_lcy
    ...
    _ax0,_ay,_ax1=spec.get("arrow",[720,540,880]); _aw=spec.get("arrow_w",4); _ahl,_ahh=spec.get("arrow_head",[22,12])
    d.line([(_ax0,_ay),(_ax1,_ay)],fill=CHALK+(210,),width=_aw); d.polygon([(_ax1,_ay),(_ax1-_ahl,_ay-_ahh),(_ax1-_ahl,_ay+_ahh)],fill=CHALK+(230,))
    _qs=spec.get("q_size",72); _qx,_qy=spec.get("q_pos",[762,468]); d.text((_qx,_qy),"？",font=_ttf(_qs),fill=GOLD+(255,))
    ril=_fit(Image.open(rp).convert("RGBA"),_rmax[0],_rmax[1]); pd=26; label_h=58
    fw=ril.size[0]+2*pd; fh=ril.size[1]+2*pd+label_h; fx0,fy0=_rcx-fw//2,_rcy-fh//2
...
def _compose_single(spec):
    ...
    import hashlib
    FX.mkdir(exist_ok=True)
    sig = hashlib.md5((repr(sorted(spec.items())) + "|" + _code_sig(_compose_single)).encode("utf-8")).hexdigest()[:10]
...
def _compose_gallery(spec):
    ...
    import hashlib, json
    FX.mkdir(exist_ok=True)
    sig = hashlib.md5((json.dumps(spec, sort_keys=True, ensure_ascii=False) + "|" + _code_sig(_compose_gallery)).encode("utf-8")).hexdigest()[:10]
...
def compose_figure(spec):
    ...
    FX.mkdir(exist_ok=True)
    # 本編は簡易版(main_simple)があれば優先＝「詳細マスター→情報を伏せた簡易版を動画に使う」恒久ルール。
    # 簡易版ファイルが無い間は詳細mainにフォールバック（Bが_simple.pngを納品したら自動で切替わる）。
    _ms = spec.get("main_simple")
    _is_simple = bool(_ms and (Path(DB) / _ms).exists())   # 簡易版は自前でラベルを焼込み済＝上乗せしない
    _main_rel = _ms if _is_simple else spec.get("main", "")
    main_p = Path(DB) / _main_rel
    if not main_p.exists():
        print(f"[warn] compose_figure: main画像が無い {main_p}"); return None
    _inset_rel = spec.get("inset", "")
    _inset_p = Path(DB) / _inset_rel if _inset_rel else ""
    # 元画像(main/inset)の更新時刻をキーに含め、差し替え時に自動再生成（stale固着の根治）。
    sig = hashlib.md5((repr(sorted(spec.items())) + "|" + _mtime_sig(main_p, _inset_p) + "|v2simple=" + str(_is_simple)).encode("utf-8")).hexdigest()[:10]
    ...
    def _label_width(f, text, ion, charge):
        ...
        return w
    for lb in ([] if _is_simple else spec.get("labels", [])):   # 簡易版は自前ラベルのみ＝上乗せ抑止(二重化防止)
    ...
    for z in ([] if _is_simple else spec.get("zones", [])):
    ...
    _notes = [] if _is_simple else spec.get("note", [])    # ※やさしい注釈（専門語を初学者向けに）＝右下の隅に小さく
    if _notes:
...
def make_ed_card(next_img=None, next_no="", next_sub="", tagline="", channel_name=""):
    ...
    import hashlib
    FX.mkdir(exist_ok=True)
    sig = hashlib.md5(f"{next_img}|{next_no}|{next_sub}|{tagline}|{channel_name}|edv2board".encode("utf-8")).hexdigest()[:8]
...
def make_slide_card(src):
    ...
    FX.mkdir(exist_ok=True)
    out = _card_out("slide", make_slide_card, src)   # コード指紋+src mtimeでstale根治（今日の誤報の本丸）
    if out.exists(): return out
    im = Image.open(src).convert("RGBA"); w, h = im.size
...
PANEL_VER = "paper3"   # サイドパネル描画の版（3種：用語解説=金/豆知識=黄メモ/補足=水色）。paper3＝左マージン線廃止＋既定縮小(PANEL_SCALE)。仕様変更で上げてキャッシュ無効化
PANEL_SCALE = 0.82     # パネル既定縮小率（ユーザー要望2026-08-14：TPOで縮小したサイズを既定に→空いた左幅でメイン図を拡大）
...
def _panel(tag, badge, accent, title, sub, note, key, subhead=None, illust_path=None, illust_cap=None):
    ...
    # 罫線（軽く）。※左マージン線は種別カラーの枠と二重に見える(ユーザー指摘「黒い線2本目はいらん」)ため廃止。
    yy=body_top-8
    ...
    if PANEL_SCALE != 1.0:   # 既定縮小＝生成後に一括リサイズ（配置/スタックはimgsizeで自動追従）
        im = im.resize((max(1,int(im.width*PANEL_SCALE)), max(1,int(im.height*PANEL_SCALE))), Image.LANCZOS)
    im.save(out); return out
...
def effective_card_image(ln):
    """カードとして実際に画面へ出る"主画像"パスを返す（表示ループの合成優先と同じ規則）。
    COMPOSITIONS該当行は合成図(_compose_/_locator_)のパスを返す＝キーワード違いでも
    同じ合成図なら"同一カード"と判定できる。重複判定(imaged)と驚きズーム複製(card_ranges)を
    これで統一し、同じ図が別カード扱いされてフェード/ワイプ/生画像フラッシュする不具合を防ぐ。"""
    _kw = str(ln.get("image_keyword", ""))
    if _kw in COMPOSITIONS:
        try:
            cf = compose_figure(COMPOSITIONS[_kw])
        except Exception:
            cf = None
        if cf: return cf
    return resolve_image(ln)
...
def main():
    ...
    ap.add_argument("--force", action="store_true", help="提出前ゲートFAILでもビルドを強制（既知の許容時のみ）")
    a = ap.parse_args(); noaudio = a.noaudio; lipsync = a.lipsync
    # ── 提出前ゲート（ビルド前に自動実行＝検品の"実行漏れ"をコードで強制） ──
    #    ・FAIL → ビルド中止(exit2)。--forceでも"ゲートは必ず実行・audit記録"し、exitだけ抑制(何を飛ばしたか残す)。
    #    ・checkdata欠落/import失敗等の"検品インフラ故障" → フェイルクローズで中止(exit3)。--forceでのみ続行。
    # ── 検品プリフライト（裏取り柱の生死を毎ビルド頭出し＝サイレント劣化防止・多AI提言B/2026-08-15） ──
    try:
        import check_system as _cs
        _keys = _cs.check_keys_only()
        _down = [n for (n, t, ok) in _keys if t == "REQUIRED" and not ok]
        if _down:
            print("🔴 [検品プリフライト] 稼働不能の裏取り柱: " + " / ".join(_down)
                  + "  → これらのAI検品は『未検証』扱い（合格を名乗れない）。要APIキー/ADC。")
        else:
            print("✅ [検品プリフライト] 裏取りの柱(ANTHROPIC/MediSearch/Vertex)は健全。")
    except Exception as _e:
        print(f"[warn] 検品プリフライト不可（check_system未実行）: {_e}")
    # ── 方針の強制簿プリフライト（記録止まりを毎ビルド頭出し＝セッション変更/ライブ速度でも貫通・見逃し不能） ──
    try:
        import policy_gate as _pg, json as _pj
        _reg = _pj.load(open(os.path.join(os.path.dirname(__file__), "checkdata", "policy_enforcement.json"), encoding="utf-8"))
        _pres = _pg.evaluate(_reg.get("policies", []))
        _pf = [pid for lv, pid, _m in _pres if lv == "FAIL"]
        _pw = [pid for lv, pid, _m in _pres if lv == "WARN"]
        if _pf:
            print("🔴 [方針プリフライト] 記録止まり/有言不実行/期限切れ: " + " / ".join(_pf)
                  + "  → 方針を放置している。`py src/policy_gate.py`で決定/実装を。")
        elif _pw:
            print("🟡 [方針プリフライト] 追随債務(期限内): " + " / ".join(_pw) + "  → 期限までにコード化を。")
        else:
            print("✅ [方針プリフライト] 全方針に決定あり・追随債務なし。")
    except Exception as _e:
        print(f"[warn] 方針プリフライト不可: {_e}")
    _gate_ok = None
    try:
        import pre_submit_gate as _psg
        print("=" * 66 + "\n🚦 提出前ゲート（ビルド前・自動実行）\n" + "=" * 66)
        _gate_ok = _psg.run_gate(str(EP_ROOT), emit_audit=True)   # 常に実行・audit.json記録
    except SystemExit:
        raise
    except Exception as _e:
        print(f"🔴 提出前ゲートを実行できませんでした（検品未実施）: {_e}")
        print("   checkdata欠落/配置ミスの可能性。修正して再実行するか、承知の上なら --force。")
        if not a.force:
            sys.exit(3)   # インフラ故障もブロック（フェイルクローズ）
        print("⚠ --force：検品不能のまま強制ビルドします。")
    if _gate_ok is False and not a.force:
        print("\n🔴 提出前ゲートで FAIL を検出 → ビルド中止。")
        print("   台本/図を修正して再実行するか、既知の許容なら --force。")
        sys.exit(2)
    if _gate_ok is False and a.force:
        print("⚠ --force：FAILを承知で強制ビルド（audit.jsonに記録済み）。")
    # ── 図レイアウトの決定論チェック（figlint）＝ラベルの重なり/見切れ/小文字を毎ビルド機械検知（記憶頼みを卒業） ──
    try:
        import figlint as _fl
        print("=" * 66 + "\n🔎 figlint（図レイアウト検査）\n" + "=" * 66)
        _fl_ok, _fl_reports = _fl.lint_build_figures()
        _fl._print_reports(_fl_reports)
        if not _fl_ok and not a.force:
            print("\n🔴 figlintで図レイアウトFAIL（ラベル重なり/見切れ）→ ビルド中止。図を修正するか既知の許容なら --force。")
            sys.exit(2)
        if not _fl_ok and a.force:
            print("⚠ --force：図レイアウトFAILを承知で強制ビルド。")
    except SystemExit:
        raise
    except Exception as _e:
        print(f"[warn] figlint実行不可（図レイアウト未検査）: {_e}")
    ...
    if ed_card and Path(ed_card).exists():
        ...
        _hops = max(12, int(_ec_len / 20))                       # 連続バウンス（回数UP＝より速く激しく：34→20）
        _amp = 96                                                # バウンス振幅UP（52→96・大きく跳ねる）
        _ai["Y"] = anim_multi([(_by if k % 2 == 0 else _by - _amp) for k in range(_hops * 2 + 1)])
        _ai["Zoom"] = anim_multi([(116.0 if k % 2 == 0 else 100.0) for k in range(_hops * 2 + 1)])  # 着地(タップ)で拡大パルス＝勢いを強調
        items.append(_ai)
    ...
    for i in range(len(spans)):
        ...
        im = effective_card_image(spans[i][0])   # ★生画像でなく"実表示の合成図"で重複判定（id3-5蝶々/id12-13NISを1カードに統合）
        if not im: continue
    ...
    # 黒板消しワイプ：ユーザー要望(2026-08-14)で"チョーク粉の粒々"オーバーレイは廃止＝無地クロスフェードに統一。
    #   スライド変更時に暗い粒々背景が一瞬見えるのを嫌ったため。WIPE_DUST=Trueで従来の粉ワイプに戻せる。
    WIPE_DUST = False
    if WIPE_DUST:
    ...
    for _ci, (ln, spk, wav, f, length) in enumerate(spans):
        ...
        _term = _isup.get("term", "実際の構造")
        if _structraw and _structraw[-1][2] == _term: continue   # 直前と同じ実構造補足(例:NISの本当の姿)＝重複表示させない(1回持続)
        _structraw.append((_sstart, str(Path(DB) / _cmp["inset"]),
                           _term, _isup.get("note", ""), _cmp.get("inset_cap", "")))
    ...
    for _si2, (f, term, note) in enumerate(_supraw):
        ...
        if _se > _sf: _sup_iv.append((_sf, _se, imgsize(make_supplement(term, note))[1]))
    def _has_sup(a, b): return any(s < b and a < e for (s, e, _h) in _sup_iv)   # [a,b)に補足が重なるか（2枚同時＝上へずらす条件）
    # 用語解説＋補足が縦に収まらない時、両方を同率で縮小して重なりを防ぐ（下端852＝立ち絵回避）
    PANEL_FLOOR = 852
    def _fit_k(h_top, h_bot):
        need = PANEL_TOP_STACK + h_top + 28 + h_bot
        return 1.0 if need <= PANEL_FLOOR else max(0.6, (PANEL_FLOOR - PANEL_TOP_STACK - 28) / (h_top + h_bot))
    def _sup_h_over(a, b):
        hs = [h for (s, e, h) in _sup_iv if s < b and a < e]
    ...
    for t, (f, abbr, full, note) in enumerate(_gl):
        ...
        if _has_sup(gf, end):
            _gtopY = PANEL_TOP_STACK; _gk = _fit_k(gh, _sup_h_over(gf, end))   # 補足と同時＝上へ(72)＋収まらなければ縮小
        else:
            _gtopY = PANEL_TOP; _gk = 1.0
        items.append(image_item(gimg, gf, max(1, end - gf), L_CHAP, *px_center(45 + gw*_gk/2, _gtopY + gh*_gk/2), 100.0*_gk, fade=0.3))  # 用語解説 X45（2枚同時は上ずらし＋縮小）
        n_gloss += 1
    ...
    for t, (f, term, note) in enumerate(_supraw):
        ...
        _sty = PANEL_TOP; _sk = 1.0
        for (_gs, _ge2, _gh, _gtopY) in _gl_iv:                  # 用語解説の下（用語解説は2枚同時で上ずらし済＝その下端の下）
            if _gs < end and sf < _ge2:
                _sk = _fit_k(_gh, sh)                            # 用語解説と同率で縮小（重なり防止）
                _sty = max(_sty, _gtopY + _gh*_sk + 28)          # 縮小後の用語解説下端＋間隔
        for (_ts, _te2, _tbot) in _tv_iv:                        # 豆知識の下
            if _ts < end and sf < _te2: _sty = max(_sty, _tbot + 28)
        _sty = PANEL_TOP if (_sty == PANEL_TOP and _sk == 1.0) else max(PANEL_TOP_STACK, min(_sty, 852 - sh*_sk))   # 立ち絵回避で下端頭打ち
        items.append(image_item(simg, sf, max(1, end - sf), L_CHAP, *px_center(45 + sw*_sk/2, _sty + sh*_sk/2), 100.0*_sk, fade=0.3))
        n_sup += 1
