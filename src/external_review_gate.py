...
def _gpg_ok(record_path: Path) -> bool:
    ...
    if sig is None:
        return False
    try:
        # 方式：署名を検証しつつ、VALIDSIG の指紋が「信頼公開鍵の指紋」とピン一致するかを厳密確認する
        #  （CIワークフローと同じ考え方）。＝別鍵の署名を弾く。
        #  ・親側の復号は utf-8+replace 固定（gpg出力のcp932デコード事故を根絶）。
        #  ・armored .asc の --keyring 直指定/一時home import は Git同梱gpgのkeyboxd事情で不安定なため採らない。
        def _run(args):
            return subprocess.run(["gpg"] + args, capture_output=True,
                                  encoding="utf-8", errors="replace", timeout=30)

        # 1) 信頼公開鍵の指紋を show-only で抽出（keyring非書込）。
        imp = _run(["--with-colons", "--import-options", "show-only", "--import", str(TRUST_PUBKEY)])
        expected = ""
        ...
        if not expected:
            return False
        # 2) 署名を status-fd 検証し、VALIDSIG の指紋を取り出してピン照合。
        ver = _run(["--status-fd", "1", "--verify", str(sig), str(record_path)])
        if ver.returncode != 0:
            return False
        for line in ver.stdout.splitlines():
            parts = line.split()
            if len(parts) >= 3 and parts[1] == "VALIDSIG" and parts[2].upper() == expected.upper():
                return True
        return False
    except Exception:
        return False
