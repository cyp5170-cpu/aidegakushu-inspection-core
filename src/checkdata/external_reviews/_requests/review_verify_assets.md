# 外部AI敵対レビュー依頼：`src/verify_assets.py`
...
## このファイルの役割
verify_core を本PJへ配線する adapter＋人サインオフ支援。effective_card_image で『実際に表示される絵』を指紋化し、text/画像変更で自動 stale 化。--signoff は人のみ。

## このファイル固有の脅威（重点的に突く）
1. 行×画像の対応ミス（使われない旧jpgを誤照合＝過去の実バグの再発）。
2. 『Claudeが代理サインオフしない』設計が破れていないか（自己申告の穴）。
3. text や画像が変わった後に確実に stale（要再確認）へ落ちるか。
...
## レビュー対象コード：`src/verify_assets.py`
- SHA256（改行正規化後・ゲートの署名対象と同一計算）：`a4af16dcd5d39315e342bab16074667f9aca8ff8dbb70a161d970378ca5ef986`
- ↓このコード全文をレビューしてください。
